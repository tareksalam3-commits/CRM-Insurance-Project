...
import { extractFormFieldSchema } from '../../customerDataExtraction/utils/formFieldSchema';
import { documentFileToImages, type ExtractionFileKind } from '../../customerDataExtraction/utils/documentToImages';
import { clearAllHighlights, highlightLowConfidenceFields } from '../../customerDataExtraction/utils/highlightFields';
import { extractPolicyDataFromDocument } from '../services/policyDocumentExtractionService';
import type { DetectedFormField } from '../../customerDataExtraction/types';

interface ExtractPolicyDataButtonProps {
...
// زر "استخراج بيانات الوثيقة بالذكاء الاصطناعي" — يظهر فقط فى حالة إصدار
// وثيقة جديدة (لا يظهر فى تعديل وثيقة موجودة). نفس أسلوب وآلية عمل زر
// استخراج بيانات العميل بالضبط (المرحلة الثانية): يقرأ حقول نموذج الوثيقة
// الحالي ديناميكياً من الـ DOM (formRef) — بما فى ذلك أي حقول مقفولة/مخفية
// حالياً (مثل مبلغ التأمين وطريقة السداد عند تفعيل customerDefaultsLocked)
// يتم تجاهلها تلقائياً لأنها لا تظهر كحقول قابلة للتعبئة — ويستخدم منظومة
// الذكاء الاصطناعي المركزية لملء النموذج فقط، بدون أي حفظ تلقائي فى قاعدة
// البيانات أو إنشاء أي سجل.
export function ExtractPolicyDataButton({ formRef, setValue }: ExtractPolicyDataButtonProps) {
  ...
  async function handleFileSelected(file: File | undefined, kind: ExtractionFileKind) {
    ...
    if (!formEl) {
      setStatus({ kind: 'error', message: 'تعذر الوصول إلى نموذج الوثيقة، أعد فتح الشاشة وحاول مرة أخرى' });
      return;
    }
    ...
    try {
      ...
      const result = await extractPolicyDataFromDocument(images, fields);
      ...
      for (const [name, { value, confidence }] of entries) {
        ...
        if (!field) continue;
        setValue(name as keyof PolicyFormData, coerceValueForField(field, value) as never, {
          shouldValidate: true,
          shouldDirty: true,
        });
  ...
  return (
    <div className="form-group">
      ...
      >
        ...
        <span>{loading ? 'جاري استخراج البيانات...' : 'استخراج بيانات الوثيقة بالذكاء الاصطناعي'}</span>
      </button>
      ...
      {sheetOpen && (
        <AppBottomSheet title="استخراج بيانات الوثيقة" subtitle={<p className="text-xs text-secondary-500 mt-0.5">اختر مصدر المستند</p>} onClose={() => setSheetOpen(false)}>
